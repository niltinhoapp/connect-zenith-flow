...
const stages = new Set<CommerceStage>([
  "discovery", "collecting_items", "collecting_fulfillment", "collecting_address",
  "collecting_payment", "awaiting_confirmation", "confirmed", "handoff",
]);
...
export function normalizeCommerceAnalysis(value: unknown): CommerceAnalysis {
  ...
  const calculatedChange = total !== null && cashFor !== null && cashFor >= total
    ? cashFor - total
    : null;
  const rawItems = Array.isArray(input.items) ? input.items : [];
  return {
    intent: intents.has(String(input.intent)) ? input.intent as CommerceAnalysis["intent"] : "other",
    ...
    fulfillment: fulfillment.has(String(input.fulfillment)) ? input.fulfillment as "delivery" | "pickup" : null,
    address: text(input.address, 500) || null,
    paymentMethod: payments.has(String(input.paymentMethod)) ? input.paymentMethod as "pix" | "card" | "cash" : null,
    cashForCents: cashFor,
    orderTotalCents: total,
    changeCents: calculatedChange,
    confirmed: input.confirmed === true,
    missingFields: (Array.isArray(input.missingFields) ? input.missingFields : []).slice(0, 10).map((item) => text(item, 120)).filter(Boolean),
    needsHuman: input.needsHuman === true,
    confidence: confidence.has(String(input.confidence)) ? input.confidence as CommerceAnalysis["confidence"] : "low",
    suggestedReply: text(input.suggestedReply, 1200),
    warnings: (Array.isArray(input.warnings) ? input.warnings : []).slice(0, 8).map((item) => text(item, 200)).filter(Boolean),
  };
}
...
export function formatCommerceAnalysis(result: CommerceAnalysis): string {
  ...
  return [
    ...
    result.missingFields.length ? `Ainda falta: ${result.missingFields.join(", ")}` : "Dados essenciais completos.",
