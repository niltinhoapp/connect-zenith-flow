...
Configurações do workspace: perfil, time/permissões (RBAC) e integrações.

- **Module key:** `configuracoes` — registered in `src/config/modules.ts`.
- **Current screens:** /configuracoes (in `src/routes/`, unchanged during F0).
- **Activated in:** F1.
