  ...
  getBoard(): Promise<CrmBoard> {
    return guard(async () => {
      ...
      const [{ data: stages, error: sErr }, { data: deals, error: dErr }] = await Promise.all([
        ...
        this.db
          .from("deals")
          .select("id, title, amount, currency, stage_id, customer_id, owner_id, tags, created_at")
          .eq("pipeline_id", pipelineId)
          .is("deleted_at", null)
      ...
      const customerIds = [...new Set(dealRows.map((d) => d.customer_id).filter((v): v is string => Boolean(v)))];
      const ownerIds = [...new Set(dealRows.map((d) => d.owner_id).filter((v): v is string => Boolean(v)))];

      const [{ data: customers }, { data: profiles }] = await Promise.all([
        customerIds.length
          ? this.db.from("customers").select("id, first_name, last_name, company_name, type").in("id", customerIds)
          : Promise.resolve({ data: [] as { id: string; first_name: string | null; last_name: string | null; company_name: string | null; type: string }[] }),
        ownerIds.length
