...
export class DealApplicationService {
  ...
  private async publishDurable(
    name: "deal.created" | "deal.updated" | "deal.stage.changed",
    payload: Record<string, unknown>,
  ) {
    if (!this.db) return;
    try {
      await publishDurable(this.db, name, {
        organizationId: this.ctx.organizationId,
        ...payload,
      } as never);
    } catch {
      /* o outbox não deve transformar uma gravação concluída em falha para o atendente */
    }
  }
  ...
  update(id: string, changes: Parameters<Deal["updateDetails"]>[0]): Promise<Deal> {
    return guard(
      async () => {
        ...
        await eventBus.publish("deal.updated", {
          organizationId: saved.organizationId,
          dealId: saved.id,
        });
  ...
  moveStage(id: string, stageId: string, stageType: StageType, reason?: string): Promise<Deal> {
    return guard(
      async () => {
        ...
        await this.publishDurable("deal.stage.changed", {
          dealId: saved.id,
          fromStageId,
          toStageId: stageId,
        });
