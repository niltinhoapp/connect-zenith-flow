...
describe("automation schedule", () => {
  it("normaliza um intervalo válido", () => {
    expect(parseSchedule({ mode: "interval", every: 3.8, unit: "hours" }))
      .toEqual({ mode: "interval", every: 3, unit: "hours" });
  });

  it("aceita o formato legado de intervalo sem mode", () => {
    expect(parseSchedule({ every: 15, unit: "minutes" }))
      .toEqual({ mode: "interval", every: 15, unit: "minutes" });
  });
  ...
  it("calcula a próxima execução por intervalo", () => {
    ...
    expect(nextRunAt({ mode: "interval", every: 90, unit: "minutes" }, from)?.toISOString())
      .toBe("2026-08-09T11:30:00.000Z");
  });

  it("calcula um horário diário futuro no mesmo dia", () => {
    const from = new Date("2026-08-09T10:00:00.000Z");
    expect(nextRunAt({ mode: "daily", at: "18:30" }, from)?.toISOString())
  ...
  it("avança para o dia seguinte quando o horário diário já passou", () => {
    ...
    expect(nextRunAt({ mode: "daily", at: "08:00" }, from)?.toISOString())
