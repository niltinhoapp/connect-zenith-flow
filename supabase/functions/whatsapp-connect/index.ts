...
async function graph(path: string, token: string, init?: RequestInit) {
  ...
  if (!res.ok) throw new Error(`graph ${path} ${res.status}: ${JSON.stringify(data?.error ?? data)}`);
  return data;
}
...
Deno.serve(async (req) => {
  ...
  const { data: canConnect, error: permErr } = await asUser.rpc("has_permission", {
    org: organizationId, perm: "whatsapp.connect",
  });
  ...
  try {
    ...
    if (mode === "embedded") {
      ...
      if (!APP_ID || !APP_SECRET) return json({ error: "META_APP_ID/SECRET não configurados" }, 400);
    ...
    const phone = await graph(`${phoneNumberId}?fields=display_phone_number,verified_name,quality_rating`, token);
    ...
    const { data: stored, error: storeErr } = await admin.rpc("wa_store_connection", {
      p_org: organizationId, p_provider: "meta", p_waba_id: wabaId, p_business_id: businessId,
      p_name: name, p_phone_number_id: phoneNumberId, p_display: phone?.display_phone_number ?? null,
      p_verified_name: phone?.verified_name ?? null, p_access_token: token, p_app_secret: APP_SECRET || null,
      p_verify_token: VERIFY_TOKEN || null,
    });
    if (storeErr) return json({ error: storeErr.message }, 400);

    return json({ ok: true, account: stored, phone: {
      display_phone_number: phone?.display_phone_number, verified_name: phone?.verified_name,
    } });
  } catch (e) {
